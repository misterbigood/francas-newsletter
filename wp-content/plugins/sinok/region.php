...
/**
 * Get array( slug => name,[...]) of regions
 * 
 * @return array
 */
function get_regions_config() {
    ...
    foreach($terms as $region) {
        ...
        $regions[$region->slug]['id'] = $region->term_id;
    }
    return $regions;
}
...
/**
 * Display the whole region list out of the loop
 * 
 * @param int $current Optional. Is used to set a specific class to the current region in the menu
 * @return string / boolean . html list of regions / false
 */
function set_regions_list_menu($current = false)
{
    $regions = get_regions_config();
    
    if(is_array($regions) && count($regions)>0):
    /* Set the html menu-list */
    $menu_region = '<ul id="region-list">'; $rel = '';
    foreach($regions as $slug => $region):
        if($current == $slug):
            $menu_region .= '<li class="active-region"><a href="'.esc_url( get_region_link($region['id'])).'"' . $rel . '>'.$region['name'].'</a></li>';
        else:
            $menu_region .= '<li><a href="'.esc_url( get_region_link($region['id'])).'"' . $rel . '>'.$region['name'].'</a></li>';
        endif;
    endforeach;
    $menu_region.= '</ul>';
    else:
